...
import static com.github.wasiqb.coteafs.appium.config.ConfigUtil.getServer;
...
public final class AppiumServer {
    ...
    public boolean isRunning() {
        ...
        try {
            final URL status = new URL(this.service.getUrl()
                .toString()
                .replace(BROADCAST_IP_ADDRESS, "127.0.0.1") + "/status");
            new UrlChecker().waitUntilAvailable(2, TimeUnit.SECONDS, status);
        } catch (final MalformedURLException | UrlChecker.TimeoutException e) {
            return false;
        }
        return true;
    }
    ...
    public void stop() {
        ...
        if (!this.setting.isExternal() && !this.setting.isCloud()) {
            try {
                ...
                fail(AppiumServerNotStoppingError.class, "Error occured while stopping the server.",
                    e);
            }
            this.service = null;
    ...
    private void buildCapabilities() {
        ...
        setCapability(NO_RESET, Boolean.toString(this.setting.isNoReset()), this.capabilities);
        setCapability(FULL_RESET, Boolean.toString(this.setting.isFullReset()), this.capabilities);
        setCapability(NEW_COMMAND_TIMEOUT, Integer.toString(this.setting.getSessionTimeout()),
            this.capabilities);
